                      ...
                      <div className="flex flex-col md:flex-row">
                        <ScrollArea className="h-[400px] md:w-1/2 rounded-md border p-4 mr-0 md:mr-2 mb-4 md:mb-0">
                          ...
                          'choices' in model.response ? (
                            ...
                            model.response.choices.map(
                              (choice: any, choiceIndex) => (
                                <div
                        ...
                        <div className="md:w-1/2 md:ml-2">
                          ...
                          {model.response &&
                            ((typeof model.response === 'object' &&
                              'choices' in model.response &&
                              model.response.choices?.some(
                                (choice: any) =>
                                  typeof choice.message?.content === 'string' &&
                                  choice.message?.content?.includes('<svg')
                              )) ||
                              (Array.isArray(model.response)
                                ? model.response.some(
                                    (message) =>
                                ...
                                : typeof model.response === 'object' &&
                                  'content' in model.response &&
                                  typeof model.response.content === 'string' &&
                                  model.response.content?.includes(
                                    '<svg'
                                  ))) && (
                                    ...
                                    dangerouslySetInnerHTML={{
                                      __html: extractSvg(
                                        typeof model.response === 'object' &&
                                          ...
                                          : typeof model.response ===
                                              'object' &&
                                            'content' in model.response &&
                                            typeof model.response.content ===
                                              'string'
                                          ? model.response.content
                                          : ''
                                      ),
