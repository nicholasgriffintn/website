...
const benchmarks = [
  ...
  },
  {
    ...
    description: "A standardised test to see how well LLMs can solve multi-part scenarios.",
  },
  {
    id: "role-play",
    type: "text",
...
function validateBenchmarkResponse(benchmark: any, response: any) {
  ...
  if (benchmark.id === "hamster-svg") {
    const messages = Array.isArray(response) ? response : response.response || [];
...
async function fetchModelResponse(model: string, benchmark: any) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`Error fetching data for ${request.chatId}:`, response.statusText);
...
async function run() {
  try {
    const existingBenchmarks = JSON.parse(fs.readFileSync("./lib/data/ai-benchmarks.json", "utf8"));

    const newBenchmarkData = await Promise.all(
      benchmarks.map(async (benchmark) => {
        const modelsToUse = benchmark.type === "image-to-text" ? imageToTextModels : textModels;
        ...
        if (existingBenchmark) {
          const successfulModels = existingBenchmark.models.filter((m) => m.status === "success");

          const modelsToRun = modelsToUse.filter(
            (m) => !successfulModels.find((sm) => sm.model === m),
          );

          const newModelData = await processBatchWithRateLimit(modelsToRun, benchmark);
        ...
        const modelData = await processBatchWithRateLimit(modelsToUse, benchmark);
    ...
    fs.writeFileSync("./lib/data/ai-benchmarks.json", JSON.stringify(newBenchmarkData, null, 2));
  } catch (error) {
